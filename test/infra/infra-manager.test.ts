...
class FakeWorktrees implements WorktreeManager {
  removed: string[] = [];
  committedArgs: Array<{ worktreePath: string; baseSha: string }> = [];
  committed = true;
  ...
  async committedChanges(worktreePath: string, baseSha: string): Promise<boolean> {
    this.committedArgs.push({ worktreePath, baseSha });
...
test("committedChanges delegates to the worktree manager", async () => {
  ...
  wts.committed = false;
  expect(await infra.committedChanges("/grove/tasks/task_x/worktree", "abc")).toBe(false);
  expect(wts.committedArgs).toEqual([{ worktreePath: "/grove/tasks/task_x/worktree", baseSha: "abc" }]);
});
